...
# in this box.
for n in nodes:
    for i in range(len(n.getName())-1):
        if i != len(n.getName())-2:
            newName = n.getName()[:i] + n.getName()[i+1] + n.getName()[i] + n.getName()[i+2]
        else:
            newName = n.getName()[:i] + n.getName()[i+1] + n.getName()[i]

        for node in nodes[nodes.index(n):]:
            if node != n and newName == node.getName():
                g.addEdge(Edge(n, node))
