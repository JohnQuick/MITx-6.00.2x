...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in viruses:
            try:
                reproduceRes = virus.reproduce(popDensity)
                if reproduceRes:
                    self.getViruses().append(reproduceRes)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for timestep in range(300):
            rawData[timestep].append(patient.update())
    
    ys = []
    for i in range(300):
        ys.append(float(sum(rawData[i]))/float(len(rawData[i])))
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantTo(self, drug):
        ...
        otherwise.
        """
        if not self.getResistances().has_key(drug):
            return False
        else:
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            reproduceable = reproduceable and self.isResistantTo(drug)
        if reproduceable:
            if self.getMaxBirthProb() * (1 - popDensity) > random.random():
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.getViruses():
            ...
            for drug in drugResist:
                allResistant = allResistant and virus.isResistantTo(drug)
            if allResistant:
                population += 1
        return population
    ...
    def update(self):
        ...
        for virus in viruses:
            try:
                reproduceRes = virus.reproduce(popDensity, self.getPrescriptions())
                if reproduceRes:
                    self.getViruses().append(reproduceRes)
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    timesteps, treatsteps = 300, 150
    totalVirusPop, resistVirusPop = [], []
    for step in range(timesteps):
        totalVirusPop.append(0)
        resistVirusPop.append(0)
    ...
    for trial in range(numTrials):
        ...
        for step in range(timesteps):
            ...
            patient.update()
            totalVirusPop[step] += patient.getTotalPop()
            resistVirusPop[step] += patient.getResistPop(['guttagonol'])

    for step in range(timesteps):
        totalVirusPop[step] = float(totalVirusPop[step])/numTrials
        resistVirusPop[step] = float(resistVirusPop[step])/numTrials

    pylab.plot(totalVirusPop, label='Total Virus Pop.') 
    pylab.plot(resistVirusPop, label='Resistant Virus Pop.') 
