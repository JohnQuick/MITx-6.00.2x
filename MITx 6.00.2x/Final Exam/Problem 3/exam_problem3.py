...
def rabbitGrowth():
    ...
    # TO DO
    probRabbitReproduction = 1.0 - float(CURRENTRABBITPOP)/MAXRABBITPOP
    newRabbit = 0
    for rabbit in range(CURRENTRABBITPOP):
        if random.random() <= probRabbitReproduction:
            newRabbit += 1
...
def foxGrowth():
    ...
    # TO DO
    probFoxEatsRabbit = float(CURRENTRABBITPOP)/MAXRABBITPOP
    for fox in range(CURRENTFOXPOP):
        if random.random() <= probFoxEatsRabbit:
